...
export const create: { body: ObjectSchema } = {
  body: Joi.object().keys({
    type: Joi.string()
      .valid(GiftCardTypeEnum.whoeverBought, GiftCardTypeEnum.specificCustomer)
      .required(),
    customerId: Joi.string().custom(objectId).when("type", {
      is: GiftCardTypeEnum.specificCustomer,
      then: Joi.required(),
      otherwise: Joi.forbidden(),
    }),
    giftCardAmount: Joi.number().required(),
    giftCardName: Joi.string().required(),
    giftCardExpiryDate: Joi.string().lowercase().required(),
  }),
}
