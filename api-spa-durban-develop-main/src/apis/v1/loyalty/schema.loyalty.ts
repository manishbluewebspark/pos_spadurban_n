...
export interface LoyaltyModel extends mongoose.Model<LoyaltyDocument> {
  paginate: (
    filter: any,
    options: any
  ) => Promise<{
...
const LoyaltySchema = new mongoose.Schema<LoyaltyDocument>(
  {
    loyaltyProgramName: {
      type: String,
      required: true,
