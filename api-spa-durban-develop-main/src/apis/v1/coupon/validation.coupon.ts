...
export const create: { body: ObjectSchema } = {
  body: Joi.object().keys({
    type: Joi.string()
      .valid(CouponTypeEnum.couponCode, CouponTypeEnum.referralCode)
      .required(),
    user: Joi.string().custom(objectId).when("type", {
      is: CouponTypeEnum.referralCode,
      then: Joi.required(),
      otherwise: Joi.forbidden(),
    }),
    earnPoint: Joi.number().when("type", {
      is: CouponTypeEnum.referralCode,
      then: Joi.required(),
      otherwise: Joi.forbidden(),
    }),
    ...
    valid: Joi.string().lowercase().required(),
    note: Joi.string().lowercase().allow(""),
  }),
}
